...
class Scheduler<T> {
    ...
    T next() {
        Map.Entry<Date,T> next = this.map.higherEntry(new Date());
        if(next == null)
            return null;
        return next.getValue();
    }
    T last() {
        Map.Entry<Date,T> next = this.map.floorEntry(new Date());
        if(next == null)
            return null;
        return next.getValue();
    }
    ArrayList<T> getAll(Date begin,Date end) {
        SortedMap<Date,T> submap = this.map.subMap(begin,true, end, true);
        return new ArrayList<>(submap.values());
    }
    T getFirst() {
        Map.Entry<Date,T> first = map.firstEntry();
        if(first == null)
            return null;
        return first.getValue();
    }
    T getLast() {
        Map.Entry<Date,T> last = map.lastEntry();
        if(last == null)
            return null;
