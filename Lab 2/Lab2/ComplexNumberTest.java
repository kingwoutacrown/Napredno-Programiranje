...
class ComplexNumber<T extends Number,U extends Number> implements Comparable<ComplexNumber<T,U>> {
    ...
    double modul() {
        return Math.sqrt((this.real.doubleValue()*this.real.doubleValue())+(this.imaginary.doubleValue()*this.imaginary.doubleValue()));
    }

    @Override
    public int compareTo( ComplexNumber<T, U> o) {
        double value =  this.modul() - o.modul();
        if(value > 0) return 1;
    ...
    public String toString() {
        ...
        sb.append(String.format("%.2f",this.real.doubleValue()));
        if(this.imaginary.doubleValue()>=0)
            sb.append("+");
        sb.append(String.format("%.2fi",this.imaginary.doubleValue()));
        return sb.toString();
        //return String.format("%.2f%si",(Double) this.real,this.imaginary);
    }
}
